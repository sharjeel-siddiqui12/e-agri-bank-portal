...
import styles from "./LoanDisbursements.module.css";
import SortArrows from "@/components/ui/sort-arrows";

/* ------------------------------ Demo Data ------------------------------ */
const REGIONS = [
  { main: "Gujrat", sub: "Kharian" },
  { main: "Gujrat", sub: "Sara-i-Alamgir" },
  { main: "Sialkot", sub: "Daska" },
...
const APP_STATUSES = ["Approved"]; // Disbursements are approved
const KYC_STATUSES = ["Approved"]; // In screenshot, all approved
...
function createLoanDisbursementsDemoData(count = 333) {
  const rows = [];
  for (let i = 1; i <= count; i++) {
    const name = NAMES[i % NAMES.length];
    const region = REGIONS[i % REGIONS.length];
    const status = APP_STATUSES[0]; // Approved
...
const demoData = createLoanDisbursementsDemoData();

/* -------------------------- Filtering & Sorting ------------------------- */
const PAGE_SIZE_OPTIONS = [25, 50, 100];
const STATUS_FILTERS = ["All Status", "Approved"];
  ...
  return (
    <div className={styles.bg}>
      <div className={styles.wrapper}>
        <h1 className={styles.heading}>Loan Disbursements</h1>
