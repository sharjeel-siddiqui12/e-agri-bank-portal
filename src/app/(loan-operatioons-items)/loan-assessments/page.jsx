...
import styles from "./LoanAssessment.module.css";
...
const KYC_STATUSES = ["Pending", "Approved"];
const APP_STATUSES = ["In-review", "Queue"];
const RISK_SCORES = ["10/9", "9/9", "8/9", "7/9"];
...
function createLoanAssessmentsDemoData(count = 333) {
  const rows = [];
...
const demoData = createLoanAssessmentsDemoData();

/* -------------------------- Filtering & Sorting ------------------------- */
const PAGE_SIZE_OPTIONS = [25, 50, 100];
const STATUS_FILTERS = ["All Status", "In-review", "Queue"];
...
function filterRows(rows, q, status) {
  ...
  return rows.filter((r) => {
    ...
    const hay = `${r.id} ${r.name} ${r.region.main} ${r.region.sub ?? ""} ${r.loanType} ${r.kyc} ${r.risk}`.toLowerCase();
...
export default function LoanAssessmentsPage() {
  ...
  function downloadCsv() {
    ...
    a.href = url;
    a.download = "loan-assessments.csv";
  ...
  return (
    <div className={styles.bg}>
      <div className={styles.wrapper}>
        <h1 className={styles.heading}>Loan Assessments</h1>
        ...
        <div className={styles.tableOuter}>
          <Table>
              ...
              <TableRow className={styles.tableHeaderRow}>
                ...
                >
                  <span className={styles.tableHeading}>
                    Loan Amount{" "}
                    <SortArrows
                      order={sortField === "amount" ? sortOrder : undefined}
                    />
                  </span>
                </TableHead>
                ...
                >
                  <span className={styles.tableHeading}>
                    Risk Scoring{" "}
                    <SortArrows
                      order={sortField === "risk" ? sortOrder : undefined}
                    />
                  </span>
                </TableHead>
            ...
            <TableBody>
              {pageRows.length === 0 && (
                <TableRow>
                  <TableCell colSpan={10} className={styles.noApplicants}>
                    No loan assessments found.
                  </TableCell>
                </TableRow>
              )}
                ...
                <TableRow key={r.id + i} className={styles.tableRow}>
                  ...
                  <TableCell className={styles.baseCell}>
                    {r.status === "In-review" ? (
                      ...
                      <span className={`${styles.statusPill} ${styles.queue}`}>
                        <span className={styles.statusDotQueue} />
                        Queue
                      </span>
                    )}
                  </TableCell>
                  ...
                  <TableCell className={styles.baseCell}>
                    <span className={styles.riskWrap}>
          ...
          <div className={styles.paginationBar}>
            ...
            <div className={styles.paginationButtons}>
              ...
              <Button
                variant="ghost"
                className={`${styles.pageBtn} ${
                  page === maxPage || maxPage === 0 ? styles.pageBtnDisabled : ""
                }`}
                onClick={() => handlePageChange(page + 1)}
