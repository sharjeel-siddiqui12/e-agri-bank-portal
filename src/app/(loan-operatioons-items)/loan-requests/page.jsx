...
function createLoanRequestsDemoData(count = 333) {
  const rows = [];
  for (let i = 1; i <= count; i++) {
    const name = NAMES[i % NAMES.length];
    const region = REGIONS[i % REGIONS.length];
    const status = i % 3 === 0 ? "In-review" : "Queue";
...
const demoData = createLoanRequestsDemoData();
...
function filterRows(rows, q, status) {
  ...
  return rows.filter((r) => {
    ...
    const hay = `${r.id} ${r.name} ${r.region.main} ${r.region.sub ?? ""} ${r.loanType}`.toLowerCase();
  ...
  return (
    <div className={styles.bg}>
      <div className={styles.wrapper}>
        <h1 className={styles.heading}>Loan Requests</h1>
          ...
          <Table>
            <TableHeader>
              <TableRow className={styles.tableHeaderRow}>
                ...
                >
                  <span className={styles.tableHeading}>
                    Loan Amount <SortArrows order={sortField === "amount" ? sortOrder : undefined} />
                  </span>
                </TableHead>
            ...
            <TableBody>
              ...
              {pageRows.map((r, i) => (
                ...
                >
                  ...
                  <TableCell className={styles.baseCell}>{formatAmount(r.amount)}</TableCell>
              ...
              {(() => {
                ...
                if (maxPage <= 5) {
                  ...
                  if (page < maxPage - 2) {
                    ...
                    items.push(
                      <Button key={maxPage} variant="ghost" className={styles.pageBtn} onClick={() => handlePageChange(maxPage)}>
                        {maxPage}
                      </Button>
                    );
                  }
