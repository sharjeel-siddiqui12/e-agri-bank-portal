...
import styles from "./AuctionRequestsApproval.module.css";
...
function makeContractId(i) {
  // LC-25-0001 style
  return `LC-25-${pad(i % 10000 || 1)}`;
}
function makeAuctionId(i) {
  // 2025-0001 style
  return `2025-${pad(i % 10000 || 1)}`;
}

function createAuctionRequestsDemoData(count = 333) {
  const rows = [];
  for (let i = 1; i <= count; i++) {
    const name = NAMES[i % NAMES.length];
    const region = REGIONS[i % REGIONS.length];
    const loanType = LOAN_TYPES[i % LOAN_TYPES.length];
    const kyc = KYC_STATUSES[i % KYC_STATUSES.length];
    const cnic = makeCNIC(i);
    const loanContractId = makeContractId(i);
    const auctionId = makeAuctionId(i);
    const { date, time } = buildDate(i % 12);

    rows.push({
      id: `${i}`.padStart(6, "0"), // internal sort id if needed
      name,
      loanContractId,
      auctionId,
      cnic,
...
const demoData = createAuctionRequestsDemoData();

/* -------------------------- Filtering & Sorting ------------------------- */
const PAGE_SIZE_OPTIONS = [25, 50, 100];
const STATUS_FILTERS = ["All Status", "Approved", "Pending"];

function normalize(str) {
  return str.toLowerCase();
}

function filterRows(rows, q, kycFilter) {
  const query = normalize(q.trim());
  return rows.filter((r) => {
    const matchesKyc = kycFilter === "All Status" ? true : r.kyc === kycFilter;
    if (!query) return matchesKyc;
    const hay = `${r.name} ${r.loanContractId} ${r.auctionId} ${r.cnic} ${r.region.main} ${r.region.sub ?? ""} ${r.loanType} ${r.kyc}`.toLowerCase();
    return matchesKyc && hay.includes(query);
  });
}
...
function sortRows(rows, field, order) {
  ...
  const sorted = [...rows].sort((a, b) => {
    ...
    switch (field) {
      ...
      case "loanContractId":
        av = a.loanContractId; bv = b.loanContractId; break;
      case "auctionId":
        av = a.auctionId; bv = b.auctionId; break;
      case "cnic":
...
export default function AuctionRequestsApprovalPage() {
  ...
  const filtered = useMemo(
    () => filterRows(demoData, search, kycFilter),
    [search, kycFilter]
  );
  ...
  return (
    <div className={styles.bg}>
      <div className={styles.wrapper}>
        <h1 className={styles.heading}>Auction Requests Approval</h1>
            ...
            <TableHeader>
              <TableRow className={styles.tableHeaderRow}>
                ...
                <TableHead
                  ...
                  onClick={() => handleSort("loanContractId")}
                >
                  <span className={styles.tableHeading}>
                    Loan Contract ID{" "}
                    <SortArrows
                      order={
                        sortField === "loanContractId" ? sortOrder : undefined
                      }
                    />
                  </span>
                </TableHead>
                ...
                >
                  <span className={styles.tableHeading}>
                    Auction ID{" "}
                    <SortArrows
                      order={sortField === "auctionId" ? sortOrder : undefined}
                    />
                  </span>
                </TableHead>
            ...
            <TableBody>
              ...
              {pageRows.map((r, i) => (
                <TableRow key={r.id + i} className={styles.tableRow}>
                  ...
                  <TableCell className={styles.baseCellMono}>
                    {r.loanContractId}
                  </TableCell>
