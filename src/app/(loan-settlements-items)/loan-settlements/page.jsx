...
import styles from "./LoanSettlements.module.css";
...
function createLoanSettlementsDemoData(count = 333) {
  const rows = [];
  for (let i = 1; i <= count; i++) {
    const name = NAMES[i % NAMES.length];
    const region = REGIONS[i % REGIONS.length];
    const status = APP_STATUSES[0]; // Recovered
...
const demoData = createLoanSettlementsDemoData();

/* -------------------------- Filtering & Sorting ------------------------- */
const PAGE_SIZE_OPTIONS = [25, 50, 100];
const STATUS_FILTERS = ["All Status", "Recovered", "Approved"];
...
/* --------------------------------- Page -------------------------------- */
export default function LoanSettlementsPage() {
    ...
    try {
      if (typeof window !== "undefined" && window.sessionStorage) {
        window.sessionStorage.setItem(
          `loan-settlements:row:${row.id}`,
          JSON.stringify(row)
        );
      }
    } catch (_) {}
    router.push(`/loan-settlements/${row.id}`);
  }

  return (
    <div className={styles.bg}>
      <div className={styles.wrapper}>
        <h1 className={styles.heading}>Loan Settlements</h1>
            ...
            <TableBody>
              ...
              {pageRows.map((r, i) => (
                <TableRow key={r.id + i} className={styles.tableRow} style={{ cursor: "pointer" }} onClick={() => openDetail(r)}>
                  ...
                  <TableCell className={styles.baseCell}>
                    <span className={`${styles.settlePill} ${styles.recovered}`}>
                      <span className={styles.recoveredDot} />
                      Recovered
                    </span>
                  </TableCell>
